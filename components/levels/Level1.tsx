...
const Level1: React.FC<Level1Props> = ({ setCurrentPage, saveLevelCompletion }) => {
  ...
  const progress = currentIndex >= ALPHABET.length - 1 ? 100 : (currentIndex / (ALPHABET.length - 1)) * 100;
  ...
  useEffect(() => {
    ...
    const totalQuestions = ALPHABET.length;
    ...
    if (starsEarned > awardedStars) {
      ...
      setTimeout(() => setShowStarAnimation(false), 1500); // Match animation duration
    }
  ...
  const handleLetterClick = (isCorrectChoice: boolean) => {
    ...
    if (currentIndex >= ALPHABET.length - 1) {
      ...
      if (isSuccess) {
        ...
        const percentage = (finalCorrect / ALPHABET.length) * 100;
        if (percentage > 60) {
          stars = 2;
        } else if (percentage > 30) {
          stars = 1;
