...
const DiscriminationShape: React.FC<{ type: string; id: number; className?: string }> = ({ type, id, className = "w-24 h-24" }) => {
  ...
  // id 0 is target, 1-5 are options.
  // We'll define logic for 15 types.
  switch (type) {
    // EASY (1-5)
    case 'E1': { // Basic Cross
...
const DiscriminationPractice: React.FC<{ 
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-5xl mx-auto py-8 px-4 animate-fade-in">
      <div className="w-full flex justify-between items-center mb-12 bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
        <div className="flex flex-col">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Progress</span>
            <span className="text-2xl font-black text-slate-800 font-mono">ITEM {currentIndex + 1} / 15</span>
        </div>
      ...
      <div className="bg-white p-12 rounded-[3rem] shadow-xl border-2 border-slate-50 mb-16 w-full flex flex-col items-center relative overflow-hidden">
        <div className="absolute top-4 left-6 text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">Target Form</div>
        <div className="p-4 border-4 border-dashed border-slate-100 rounded-[2rem] bg-slate-50/30">
        ...
        {[1, 2, 3, 4, 5].map((opt) => (
          <div key={opt} className="flex flex-col items-center gap-6">
            <div 
              ...
              className={`w-full aspect-square bg-white rounded-[2rem] shadow-md border-2 flex items-center justify-center cursor-pointer transition-all hover:scale-105 active:scale-95 ${selectedOption === opt ? 'border-cyan-500 bg-cyan-50/30 shadow-cyan-100' : 'border-slate-50'}`}
            >
      ...
      <button onClick={() => setIsExitConfirmOpen(true)} className="fixed bottom-8 right-8 group w-16 h-16 rounded-full flex items-center justify-center bg-white shadow-2xl transition-transform hover:scale-110 focus:outline-none z-50">
...
const PerceptualModuleCard: React.FC<{
  ...
  delay: string;
  onBegin: () => void;
}> = ({ number, title, category, description, icon, delay, onBegin }) => (
  <div 
    className={`bg-white rounded-[1.5rem] p-8 shadow-[0_10px_40px_rgba(0,0,0,0.03)] border border-slate-100 flex flex-col relative overflow-hidden group hover:shadow-xl hover:border-cyan-100 transition-all duration-500 transform hover:-translate-y-1 animate-fade-in-up`}
    style={{ animationDelay: delay }}
  >
    <div className="absolute top-0 right-4 text-[7rem] font-black text-slate-50 opacity-[0.03] select-none pointer-events-none group-hover:opacity-[0.06] transition-opacity">
      {number}
    </div>

    <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center mb-8 shadow-inner border border-slate-100 group-hover:bg-cyan-50 group-hover:border-cyan-100 transition-colors">
    ...
    <div onClick={onBegin} className="mt-10 flex items-center justify-between group/action cursor-pointer">
      <span className="text-[11px] font-black text-slate-300 uppercase tracking-widest group-hover:text-cyan-600 transition-colors">BEGIN ITEM #{number}</span>
      <div className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-300 group-hover/action:bg-cyan-500 group-hover/action:text-white transition-all">
...
const PerceptualPage: React.FC<PerceptualPageProps> = ({ setCurrentPage, saveLevelCompletion, setPerformanceFilter }) => {
  ...
  const handleComplete = (score: number, errors: number) => {
    ...
    saveLevelCompletion('perceptual_disc', stars, { 
        score, 
        incorrect: errors, 
        category: 'percep' // Updated category to 'percep'
    });
  ...
  if (activeModule === 'summary') {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-8 animate-fade-in">
        <div className="bg-white p-12 rounded-[3rem] shadow-2xl max-w-md w-full text-center border-t-8 border-cyan-500">
          <div className="mb-8">
          ...
          <h2 className="text-3xl font-black text-slate-800 mb-2 tracking-tight uppercase">Results Summary</h2>
          ...
          <button 
            ...
            className="w-full bg-slate-900 text-white font-black py-5 rounded-[2rem] hover:bg-slate-800 transition-all flex items-center justify-center gap-3 uppercase tracking-widest text-xs mb-4"
          >
            <RetryIcon className="w-5 h-5" /> Retake Practice
          </button>
          <button 
            onClick={() => setActiveModule('list')}
            className="w-full bg-slate-100 text-slate-500 font-black py-5 rounded-[2rem] hover:bg-slate-200 transition-all uppercase tracking-widest text-xs"
          >
            Back to Modules
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#fcfdfe] flex flex-col font-sans overflow-x-hidden">
      <header className="w-full h-20 bg-white border-b border-slate-100 flex items-center justify-between px-6 sm:px-12 sticky top-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col -space-y-1">
             <span className="text-sm font-black tracking-tight text-slate-800 uppercase">VISUAL <span className="text-cyan-500">PERCEPTUAL PRACTICE</span></span>
             <span className="text-[9px] font-black text-slate-400 tracking-widest uppercase">Diagnostic Protocol TVPS-3</span>
          </div>
        </div>

        <div className="flex items-center gap-8">
           <div className="hidden md:flex items-center gap-2 bg-cyan-50 px-4 py-2 rounded-full border border-cyan-100 shadow-sm">
              <span className="w-2 h-2 rounded-full bg-cyan-500 animate-pulse"></span>
              <span className="text-[10px] font-black text-cyan-700 uppercase tracking-widest">Clinical Mode Active</span>
           </div>
           <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 font-bold text-xs border border-slate-200 cursor-pointer hover:bg-slate-200 transition-colors">
              DR
           </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto w-full px-6 sm:px-12 py-16 flex flex-col">
        <div className="text-center mb-20 animate-fade-in-up">
           <div className="inline-flex items-center gap-2 bg-cyan-50 px-6 py-2 rounded-full mb-6 border border-cyan-100">
              ...
              <span className="text-[10px] font-black text-cyan-700 uppercase tracking-[0.2em]">Diagnostic Protocol TVPS-3</span>
           </div>
           <h1 className="text-5xl font-black text-slate-900 tracking-tighter mb-4">Practice Assessment Modules</h1>
        ...
        <div className="bg-[#040a1e] rounded-[2.5rem] p-10 sm:p-14 shadow-2xl relative overflow-hidden group animate-fade-in-up" style={{ animationDelay: '0.8s' }}>
           ...
           <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
              ...
              <div className="flex flex-col items-center gap-4">
                 ...
                 <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Real-time Sync Active</span>
                 </div>
              </div>
           </div>
      ...
      <button
        ...
        className="fixed bottom-8 right-8 group w-16 h-16 rounded-full flex items-center justify-center bg-white shadow-[0_15px_45px_rgba(0,0,0,0.15)] transition-all hover:scale-110 active:scale-95 focus:outline-none z-[60] border border-slate-50"
        aria-label="Home"
      >
        <span className="absolute inset-0 rounded-full border-2 border-cyan-200 opacity-70 group-hover:border-cyan-500 group-hover:opacity-100 transition-all duration-300"></span>
